...
import { Plug, ThumbsUp, Clock, User } from 'lucide-react'

import { Badge } from '@/components/ui/badge'
import { Card, CardHeader, CardTitle, CardContent } from '@/components/ui/card'
import { MCP_SERVER_TYPES, MAX_VISIBLE_TAGS } from '@/lib/constants'
...
export const McpCard = memo(function McpCard({ mcp }: McpCardProps) {
  const visibleTags = mcp.tags.slice(0, MAX_VISIBLE_TAGS)
  const remainingTags = mcp.tags.length - MAX_VISIBLE_TAGS
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="flex items-center gap-2">
              <div className="from-primary/10 to-secondary/10 flex h-6 w-6 shrink-0 items-center justify-center rounded bg-linear-to-br">
                <Plug className="text-primary h-3.5 w-3.5" />
              </div>
              <CardTitle className="line-clamp-2 text-base md:text-lg">{mcp.title}</CardTitle>
            </div>
            {mcp.server_type && (
              <Badge variant="outline" className="shrink-0 text-xs">
                {getServerTypeLabel(mcp.server_type)}
              </Badge>
            )}
          </div>
          {mcp.category_name && (
            <p className="text-muted-foreground text-xs md:text-sm">{mcp.category_name}</p>
          )}
        </CardHeader>
        <CardContent className="space-y-3 md:space-y-4">
          {mcp.description && (
            <p className="text-muted-foreground line-clamp-2 text-sm">{mcp.description}</p>
          )}
